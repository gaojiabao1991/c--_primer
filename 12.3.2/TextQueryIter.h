...
class QueryResult {
   public:
    using QueryResultIter = set<int>::iterator;
    QueryResult(shared_ptr<set<int>> idx, StrBlob lines) : idx(idx), lines(lines) {}
    void print();
    QueryResultIter begin() { return idx->begin(); }
    QueryResultIter end() { return idx->end(); }
    shared_ptr<StrBlob> get_file() { return make_shared<StrBlob>(lines); }
