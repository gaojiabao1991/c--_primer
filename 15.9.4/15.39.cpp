...
class Query {
    ...
    Query(shared_ptr<BaseQuery> bq) : bq(bq) {}

   public:
    Query(const string& word) : bq(new WordQuery(word)) {}
...
Query operator~(const Query& wq) {
    return shared_ptr<BaseQuery>(new NotQuery(wq));
}

Query operator&(const Query& l, const Query& r) {
    return shared_ptr<BaseQuery>(new AndQuery(l, r));
}

Query operator|(const Query& l, const Query& r) {
    return shared_ptr<BaseQuery>(new OrQuery(l, r));
}
