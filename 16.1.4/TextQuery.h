...
class QueryResult {
   public:
    QueryResult(shared_ptr<set<int>> idx, shared_ptr<vector<string>> lines)
        : idx(idx), lines(lines) {}
    void print();
...
void QueryResult::print() {
    ...
    for (auto& i : *idx) {
        cout << show_num + 1 << ") line " << (i + 1) << " , " << (*lines)[i]
             << endl;
...
TextQuery::TextQuery(ifstream& ifs)
    : lines(new vector<string>(), DebugDelete("delete lines")) {
    string line;
    int idx = 0;
    while (std::getline(ifs, line)) {
        ...
        while (ss >> word) {
            ...
            shared_ptr<set<int>>& idx_pt = word2lines[word];
            if (!idx_pt) {
                idx_pt.reset(new set<int>(),
                             DebugDelete("delete idx for word: " + word));
            }
            idx_pt->insert(idx);
        }
        idx++;
